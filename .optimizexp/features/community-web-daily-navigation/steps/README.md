# Steps & tests — `community-web-daily-navigation`
...
| `../test/community-web-daily-navigation.bindings.test.ts` | Vitest smoke for bindings |
...
```bash
# unit/smoke (vitest)
pnpm run test:file .optimizexp/features/community-web-daily-navigation/test/community-web-daily-navigation.bindings.test.ts

# cucumber (optimizexp profile, if configured)
pnpm run test:behavior:optimizexp -- --name "community-web-daily-navigation"
# or:
node --import tsx node_modules/@cucumber/cucumber/bin/cucumber.js \
  --config .optimizexp/cucumber.yaml \
  --name "community-web-daily-navigation"
```
...
4. Re-run discovery: `generate-feature.mts --mode implement --id community-web-daily-navigation`.
