...
Feature: Safe reusable Community Web actions for a forge power user
  As a forge and terminal power user
  I want to compose existing Community Web commands into a named action
  ...
  Scenario: Forge power user operates every focused-post action by keyboard
    Given I open `http://127.0.0.1:8787/` and focus a post in a Community Web channel
  ...
  Scenario: Forge power user defines and runs a safe reusable macro
    ...
    And `macro list` shows review and its two existing Community Web commands
    When I run `macro run review`
    Then Community Web navigates to `/projects/epoch/changes` and applies `state:needs-review`
    And the macro remains after reloading the page

  Scenario: Forge power user gets an actionable refusal for an unsafe macro
    Given I am using the Community Web prompt
  ...
  Scenario: Forge power user expands and restores the focused panel
    Given I focus a Community Web navigation, thread, session, or editor panel
