...
Feature: Keyboard-operable Community Web messages for a screen-reader power user
  ...
  Scenario: Screen-reader power user traverses messages with one roving tab stop
    Given I open `http://127.0.0.1:8787/` and enter a Community Web text channel
  ...
  Scenario: Screen-reader power user recovers from the end of a message list
    Given I open a Community Web text channel and focus its last message
  ...
  Scenario: Screen-reader power user discovers every post action shortcut
    Given I focus a post in a Community Web text channel
