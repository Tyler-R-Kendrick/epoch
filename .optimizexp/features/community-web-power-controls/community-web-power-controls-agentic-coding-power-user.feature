...
Feature: One Nightboard action contract for agents and voice
  ...
  Scenario: Agentic coding power user consumes compatible startup conditions with one restart
    Given Nightboard detects a resumable Codex session, an available update, and unprimed workspace defaults
    Then the bottom line recommends `Ctrl+U` and names all compatible changes
    When I press `Ctrl+U` outside the file editor
    Then Nightboard applies the update and workspace defaults before continuing the session
