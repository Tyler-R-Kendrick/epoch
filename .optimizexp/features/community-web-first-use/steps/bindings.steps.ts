/**
 * Cucumber step bindings for OptimizeXP feature `community-web-first-use`.
...
function getSingleton(): OptimizexpWorld {
  singleton ??= createWorld("community-web-first-use");
  return singleton;
}
...
Given("persona {string} is active", function (this: OptimizexpWorld, personaId: string) {
  ...
  if (!world.featureId) Object.assign(world, createWorld("community-web-first-use"));
