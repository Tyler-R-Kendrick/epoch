...
interface WorldState {
  ...
  collaborationProjection?: ReturnType<EpochRepository["collaboration"]>;
  gateResult?: { passed: boolean; blockers: readonly string[] };
  reusableResolution?: JsonValue;
...
async function closeWithTimeout(task: PromiseLike<JsonValue | void> | undefined, label: string, timeoutMs = 5_000): Promise<void> {
  if (!task) {
    return;
  }
...
Then("the version manifest includes file {string}", function (expected: string) {
  ...
  assert.ok(files.some((file) => isJsonObject(file) && file.path === expected));
});
...
Then("the version manifest includes entity {string}", function (expected: string) {
  ...
  assert.ok(entities.some((entity) => isJsonObject(entity) && entity.name === expected));
});
...
Then("workspace JSON file {string} has property {string} equal to {string}", function (path: string, propertyPath: string, expected: string) {
  ...
  const data = JSON.parse(readFileSync(join(state.workspace, path), "utf8")) as JsonValue;
...
When("I tamper with the recorded event size", function () {
  ...
  // SAFETY: Runtime checks or construction above establish { payload: { size: number } }.
...
When("I create a signed issue titled {string} with body {string}", function (title: string, body: string) {
  // SAFETY: Runtime checks or construction above establish unknown as { createIssue(title: string.
  state.lastEvent = (state.repo as EpochRepository & { createIssue(title: string, body: string): Event }).createIssue(title, body);
});

When("{string} signs a review {string} on the intent with body {string}", function (author: string, stateName: string, body: string) {
  assert.ok(state.lastIntentId);
  // SAFETY: Runtime checks or construction above establish unknown as { reviewIntent(intentId: string.
  state.lastEvent = (state.repo as EpochRepository & { reviewIntent(intentId: string, state: string, body: string, author: string): Event }).reviewIntent(state.lastIntentId, stateName, body, author);
});

When("{string} records CI {string} as {string} for the intent", function (author: string, name: string, status: string) {
  assert.ok(state.lastIntentId);
  // SAFETY: Runtime checks or construction above establish unknown as { recordCI(name: string.
  state.lastEvent = (state.repo as EpochRepository & { recordCI(name: string, status: string, target: string, author: string): Event }).recordCI(name, status, state.lastIntentId, author);
});

Then("the collaboration projection contains issue {string}", function (title: string) {
  // SAFETY: Runtime checks or construction above establish unknown as { collaboration(): Record<string.
  state.collaborationProjection = state.repo.collaboration();
  ...
  assert.ok(issues.some((issue) => isJsonObject(issue) && issue.title === title));
});
...
Then("the gate status for the intent requiring review {string} and CI {string} passes", function (reviewState: string, ciName: string) {
  assert.ok(state.lastIntentId);
  // SAFETY: Runtime checks or construction above establish unknown as { gateStatus(intentId: string.
  state.gateResult = (state.repo as EpochRepository & { gateStatus(intentId: string, policy: JsonObject): { passed: boolean; blockers: readonly string[] } }).gateStatus(state.lastIntentId, {
...
When("I sync to the peer through a memory transport", function () {
  assert.ok(state.peerRepo);
  // SAFETY: Runtime checks or construction above establish unknown as { exportToMemoryTransport(): JsonValue }).exportToMemoryTransport().
  const transport = (state.repo as EpochRepository & { exportToMemoryTransport(): JsonValue }).exportToMemoryTransport();
  // SAFETY: Runtime checks or construction above establish unknown as { syncWithTransport(transport: JsonValue): SyncResult }).syncWithTransport(transport).
  state.syncResult = (state.peerRepo as EpochRepository & { syncWithTransport(transport: JsonValue): SyncResult }).syncWithTransport(transport);
});
...
When("I record a reusable conflict resolution for {string} as {string}", function (path: string, entityType: string, table: DataTable) {
  ...
  // SAFETY: Runtime checks or construction above establish unknown as {.
  state.lastEvent = (state.repo as EpochRepository & {
    recordConflictResolution(input: JsonObject): Event;
...
Then("the repository reuses the conflict resolution for {string} as {string}", function (path: string, entityType: string, table: DataTable) {
  const row = table.hashes()[0];
  // SAFETY: Runtime checks or construction above establish unknown as {.
  state.reusableResolution = (state.repo as EpochRepository & {
    reusableConflictResolution(input: JsonObject): JsonValue;
...
When("I append an operation event for command {string} with status {string}", function (command: string, status: string) {
  // SAFETY: Runtime checks or construction above establish unknown as { appendOperation(command: string.
  state.lastEvent = (state.repo as EpochRepository & { appendOperation(command: string, status: string): Event }).appendOperation(command, status);
});

Then("the operation event log contains command {string} with status {string}", function (command: string, status: string) {
  // SAFETY: Runtime checks or construction above establish unknown as { operations(): readonly Record<string.
  const operations = (state.repo as EpochRepository & { operations(): readonly JsonObject[] }).operations();
...
When("I redact the last recorded blob with reason {string}", function (reason: string) {
  ...
  // SAFETY: Runtime checks or construction above establish unknown as { redactBlob(blobHash: string.
  state.lastEvent = (state.repo as EpochRepository & { redactBlob(blobHash: string, reason: string): Event }).redactBlob(state.redactedBlobHash, reason);
});
...
Then("the redaction projection contains reason {string}", function (reason: string) {
  // SAFETY: Runtime checks or construction above establish unknown as { redactions(): readonly Record<string.
  const redactions = (state.repo as EpochRepository & { redactions(): readonly JsonObject[] }).redactions();
...
When("I initialize an Epoch repository with custom {string} serialization as {string}", function (format: string, author: string) {
  const serializer = {
    format,
    extension: `.${format}`,
    serialize: (value: JsonValue) => `format=${format}\n${JSON.stringify(value)}\n`,
...
function runCli(main: (argv: string[], io: CliIO) => number | Promise<number>, argv: string[], cwd = state.workspace): void | Promise<void> {
  ...
  try {
    ...
    if (result instanceof Promise) {
      return result.then((exitCode) => finish(exitCode), (error) => {
        finish();
        throw error;
      });
    }
...
function metadataValue(key: string): JsonValue | undefined {
  const metadata = state.lastEvent?.payload.metadata;
  assert.ok(isJsonObject(metadata));
  return metadata[key];
}

function metadataLabels(): JsonValue | undefined {
  return metadataValue("labels");
}

function valueAtPath(value: JsonValue, propertyPath: string): JsonValue | undefined {
  return propertyPath.split(".").reduce<JsonValue | undefined>((current, key) => {
    assert.ok(isJsonObject(current));
    return current[key];
  }, value);
}
...
Then("the merged text contains {string}", function (expected: string) {
  assert.ok(isString(state.merged));
  assert.match(state.merged, new RegExp(expected));
});
