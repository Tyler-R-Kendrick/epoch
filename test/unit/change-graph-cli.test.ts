...
export async function runChangeGraphCliTests(): Promise<void> {
  ...
  try {
    ...
    process.env.EPOCH_SWH_SAVE_URL = `http://127.0.0.1:${typeof archiveAddress === "object" && archiveAddress ? archiveAddress.port : 0}/`;
    try {
    ...
    const canonicalRecords = [
      ...
      (await executeChangeGraphCommand(root, ["split", "propose", (injectedRevision.data as { id: string }).id, "--plan", '{"groups":[]}'], 203, dependencies)),
      (await executeChangeGraphCommand(root, ["bundle", "create", "Canonical bundle", (injectedRevision.data as { id: string }).id], 204, dependencies)),
    ];
