...
export interface ExtensionTrustPolicy {
  ...
  /**
   * Expiry, succession, and revocation statements in force here (ADR-0045).
   *
...
/** One thing wrong with an `[extensions]` table, named by key (ADR-0047). */
  ...
  return {
    policy: {
      ...
      lifecycle: {
        ...
        // is exactly who they are defending against (ADR-0045).
...
export function evaluateTrust(
  ...
  if (policy.trust === "signed") {
    ...
    // rotated key does not have to be pasted into every clone (ADR-0045).
