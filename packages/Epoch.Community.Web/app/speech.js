/**
 * Speech-to-text for the Community Web prompt — on-device model first.
 *
...
(function () {
  ...
  function matchCommand(phrase) {
    ...
    var userAction = window.CW_POWER && window.CW_POWER.resolveVoice(t);
    if (userAction) {
    ...
    var builtInAction = window.CW_ACTIONS && window.CW_ACTIONS.resolve("voice", t);
    if (builtInAction) {
      return { kind: "action", actionId: builtInAction, input: {}, hint: "voice action" };
    }
    var nearBuiltIn = window.CW_ACTIONS && window.CW_ACTIONS.voiceCatalog().some(function (action) {
