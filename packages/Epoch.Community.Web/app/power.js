...
(function () {
  ...
  function read() {
    try {
      ...
      Object.keys(raw).forEach(function (name) {
        ...
        try {
          var migrated = window.CW_ACTIONS.migrateMacro({ commands: commands });
  ...
  function knownCommand(line) {
    ...
    return !!(window.CW_ACTIONS &&
      (window.CW_ACTIONS.resolve("cli", first) || window.CW_ACTIONS.resolve("slash", first)));
  }

  function run(name) {
    var item = actions[name];
    if (!item) return { ok: false, text: "macro: no such action: " + name };
    if (!runtime || !window.CW_ACTIONS) return { ok: false, text: "macro: runtime not ready" };
    var completion = item.actionIds.reduce(function (prior, actionId, index) {
      return prior.then(function () {
        ...
        return window.CW_ACTIONS.invoke(actionId, {
  ...
  function register(name) {
    ...
    if (!window.CW_MCP || !actions[name]) return;
    var item = actions[name];
    unregister[name] = window.CW_MCP.registerTool({
      name: "user_" + name,
      description: "User-defined Community Web skill " + name + ": " + item.commands.join("; "),
  ...
  function save(name, commandText) {
    ...
    var migrated;
    try { migrated = window.CW_ACTIONS.migrateMacro({ commands: commands }); }
