...
(function () {
  ...
  function core() {
    if (!window.CW_CORE) throw new Error("Community Core runtime must load before the Community Web namespace");
    return window.CW_CORE;
  }
   ...
   * Merges fixture notifications with hook-fired Activity from CW_HOOKS.
   */
  function allNotifications(readSet) {
    ...
    var hookFired = [];
    if (window.CW_HOOKS && window.CW_HOOKS.fired) {
      hookFired = window.CW_HOOKS.fired(readSet || null);
    }
  ...
  function allSpaces() {
    if (window.CW_SESSION && window.CW_SESSION.listSpaces) {
      return window.CW_SESSION.listSpaces();
    }
    return (D.spaces || []).slice();
  }

  function findSpaceNode(id) {
    if (window.CW_SESSION && window.CW_SESSION.findSpace) {
      return window.CW_SESSION.findSpace(id);
    }
  ...
  /** Session-created channels/projects — live overlay from CW_APP.state.boardOverlay. */
  function boardOverlay() {
    return (window.CW_APP && window.CW_APP.state && window.CW_APP.state.boardOverlay) || {};
  }
  ...
  function currentReadSet() {
    try {
      return (window.CW_APP && window.CW_APP.state && window.CW_APP.state.notifRead) || null;
    } catch {
      return null;
    }
  }
