...
(function () {
  ...
  function install(api) {
    var MCP = window.CW_MCP;
    var MAP = window.CW_MAP;
    var GRAPH = window.CW_GRAPH;
    ...
    MCP.registerTool({
      ...
      inputSchema: {
        type: "object",
        properties: { name: { type: "string", description: "grid (Grid)" } },
        required: ["name"],
      },
      execute: async function (args) {
        // "nightboard" is this theme's retired id; still resolved so saved
        // macros and agent scripts written against it keep working.
        var asked = String(args.name === "nightboard" ? "grid" : args.name);
        var i = window.CW_THEMES.findIndex(function (t) {
          return t.id === asked || t.name.toLowerCase() === asked.toLowerCase();
        });
        if (i === -1) {
          return MCP.fail("themes are: " + window.CW_THEMES.map(function (t) { return t.id; }).join(", "));
        }
        api.setTheme(i);
        return MCP.text("theme is " + window.CW_THEMES[i].name);
      },
    });
  ...
  window.CW_TOOLS = { install: install };
})();
