/**
 * Custom event hooks for Community Web.
 *
 ...
 *   - Lucene-style query when CW_QUERY is available (for post-shaped payloads)
 *
...
(function () {
  ...
  function fixtureHooks() {
    var fromData = (window.CW_DATA && window.CW_DATA.hooks) || null;
  ...
  function matchesFilter(match, payload, eventName) {
    ...
    // query.matched (and any hook with a rich query) can use CW_QUERY on post-shaped payloads.
    if (window.CW_QUERY && window.CW_QUERY.apply &&
      ...
      if (payload && (payload.body != null || payload.subject != null || payload.channel || payload.who)) {
        try {
          var result = window.CW_QUERY.apply([payload], match, {
  ...
  function whereFor(payload) {
    ...
    if (payload.channel && window.CW_MAP && window.CW_MAP.channelPath) {
      var chan = ((window.CW_DATA && window.CW_DATA.channels) || []).filter(function (c) {
        return c.id === payload.channel;
      })[0];
      if (chan) return window.CW_MAP.channelPath(chan.label);
    }
    if (payload.channel) {
