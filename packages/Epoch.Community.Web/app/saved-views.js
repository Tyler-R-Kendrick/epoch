/** Durable named query projections. Query semantics remain owned by CW_QUERY. */
(function () {
  "use strict";
    ...
    try {
      var next = {
        ...
        views: state.views.map(function (view) {
          ...
          var query = window.CW_CORE.migrateNormalizedQuery({
            query: view.query,
  ...
  function normalized(query) {
    var result = window.CW_QUERY.normalize(query);
  ...
  function open(projectionId, objects, context) {
    ...
    var result = window.CW_QUERY.apply(visible, view.query, context);
