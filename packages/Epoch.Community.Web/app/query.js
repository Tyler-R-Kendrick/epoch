/**
 * Lucene-style feed queries for Community Web views / projections.
 *
...
(function () {
  ...
  function parse(input) {
    var normalized = window.CW_CORE.normalizeQuery(String(input || ""));
  ...
  function apply(posts, query, ctx) {
    ...
    if (parsed.ast) {
      filtered = visible.filter(function (post) {
        return window.CW_CORE.evaluateSearchExpression(feedEntity(post), parsed.ast).matches;
      });
    }
  ...
  function feedEntity(post) {
    ...
    var entity = window.CW_CORE.communityMessageToEntity(message, {
      provenance: { sourceId: "community-web-feed", nativeId: objectId, observedAt: publishedAt },
  ...
  function memberKind(who) {
    var members = window.CW_DATA && Array.isArray(window.CW_DATA.members) ? window.CW_DATA.members : [];
  ...
  function presets() {
    var extra = (window.CW_DATA && window.CW_DATA.feedViews) || [];
    var seen = {};
    var out = [];
  ...
  function pathHits(terms, extra, ctx) {
    if (!terms || !terms.length || !window.CW_MAP || !window.CW_COMPLETE) return [];
    var MAP = window.CW_MAP;
    var viewer = ctx && ctx.viewer || {};
    var readableDmIds = viewer.actorId ? viewer.readableDmIds || [] : [];
    var score = window.CW_COMPLETE.score;
    var hits = [];
    var seen = {};
  ...
  function formatSearchResults(result, opts) {
    ...
    var qHtml = (window.CW_SYNTAX
      ? window.CW_SYNTAX.highlight(q, "lucene")
      : escapeHtml(q));
    var html = [
    ...
    if (m) {
      ...
      values.forEach(function (v) {
        if (!val || String(v).toLowerCase().indexOf(val.toLowerCase()) === 0 ||
            (window.CW_COMPLETE && window.CW_COMPLETE.score &&
              window.CW_COMPLETE.score(String(v), val) != null)) {
          candidates.push({
            value: field + ":" + v,
            hint: field + " value",
            kind: "field",
  ...
  window.CW_QUERY = {
    ...
    VERSION: window.CW_CORE.QUERY_LANGUAGE_VERSION,
    tokenize: tokenize,
  };
})();
