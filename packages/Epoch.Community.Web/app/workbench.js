/** Thin Community Web host for the canonical Community query and projection runtimes. */
(function () {
  "use strict";
  ...
  function openProjection(state, definition) {
    state.searchWorkbench = {
      kind: "projection", tab: "definition", source: JSON.stringify(definition || window.CW_CORE.builtinDefaultProjection, null, 2) + "\n",
  ...
  function compileProjection(state) {
    ...
    try {
      var fields = window.CW_CORE.CORE_COMMUNITY_FIELDS;
      workbench.compiled = window.CW_CORE.compileProjectionDefinition(parseProjection(workbench), {
  ...
  function saveSearchProjection(state, label) {
    ...
    window.CW_CORE.compileProjectionDefinition(definition, {
      fields: window.CW_CORE.CORE_COMMUNITY_FIELDS.map(function (field) { return field.name; }),
      sortableFields: window.CW_CORE.CORE_COMMUNITY_FIELDS.filter(function (field) { return field.sortable; }).map(function (field) { return field.name; }),
  ...
  function runSearch(state, options) {
    ...
    workbench.error = null;
    var parsed = window.CW_CORE.normalizeQuery(workbench.expression, {
    ...
    var result = window.CW_QUERY.searchBoard(parsed.canonical, options.context || {});
    var sourceCheckpoints = options.sourceCheckpoints || [
      { sourceId: "community-web-host", token: "resident", observedAt: parsed.resolvedAt, status: "current" },
    ];
