...
export interface NightboardFixtureRecord {
  readonly id?: string;
...
export interface NightboardFixtureData {
  readonly posts?: readonly NightboardFixtureRecord[];
  readonly dmMessages?: readonly NightboardFixtureRecord[];
  readonly projectPosts?: readonly NightboardFixtureRecord[];
  readonly notifications?: readonly NightboardFixtureRecord[];
  readonly members?: readonly NightboardFixtureRecord[];
}

export function createNightboardSource(data: NightboardFixtureData, observedAt: string) {
  const sourceId = "nightboard-host";
  const entities = normalizeNightboardData(data, observedAt);
  const token = `nightboard:${observedAt}:${entities.length}`;
...
export function normalizeNightboardData(data: NightboardFixtureData, observedAt: string): readonly CommunityEntity[] {
  const groups: readonly [keyof NightboardFixtureData, CommunityObjectKind, CommunityEntity["visibility"]][] = [
  ...
  const entities = groups.flatMap(([key, defaultKind, defaultVisibility]) => (data[key] ?? []).map((record) => {
    ...
    if (objectId === undefined) throw new Error(`Nightboard ${key} record has no canonical object ID`);
    const createdAt = record.publishedAt ?? record.createdAt;
    if (createdAt === undefined) throw new Error(`Nightboard ${key} record has no persisted timestamp`);
    ...
    return validateCommunityEntity({
      ...
      createdAt,
      updatedAt: record.updatedAt ?? createdAt,
      provenance: { sourceId: "nightboard-host", nativeId: record.id ?? objectId, observedAt, ...(record.revision === undefined ? {} : { revision: record.revision }) },
    });
  }));
  const ids = new Set<string>();
  for (const entity of entities) {
    if (ids.has(entity.ref.objectId)) throw new Error(`Duplicate Nightboard canonical object ID: ${entity.ref.objectId}`);
