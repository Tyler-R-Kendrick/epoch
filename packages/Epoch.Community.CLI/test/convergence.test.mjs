...
test("CLI exposes graph, weave gates, and dependency-closed preview", async () => {
  ...
  assert.equal(await main(["stack", "graph"], { stdout: (line) => output.push(line), stderr: (line) => errors.push(line) }, context), 0);
  assert.equal(await main(["review", "weave"], { stdout: (line) => output.push(line), stderr: (line) => errors.push(line) }, context), 0);
