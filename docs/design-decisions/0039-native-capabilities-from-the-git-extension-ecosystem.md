...
| Gerrit, GitButler Gerrit Mode | `epoch change` + `epoch change submit` | Review is change-based, not branch-based. A Change-Id trailer and `refs/for/<target>` are the Git dialect of an Epoch Change; they are not a mode. See [ADR-0051](0051-change-based-review-publish.md). |
