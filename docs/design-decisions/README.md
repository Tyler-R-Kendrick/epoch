...
| [ADR-0058](0058-lua-scriptable-generative-ui-customization.md) | Proposed | Lua-scriptable generative UI customization: wasmoon pooled runtime, extended OpenUI catalog + `{root, elements}` spec, DTCG `--cw-*` overlays, three-tier CONTRACT amendment, scripts as advisory `view` extensions, persisted GraphQL ops. |
| [ADR-0059](0059-live-spaces-semantic-sessions.md) | Accepted; semantic-only core implemented | Live Spaces: signed live-session protocol events and Core store over existing Spaces, fail-closed recursive publication engine with an immutable deny baseline, deterministic presentation log with checkpoints/fork eligibility, `live.*` commands on the shared bus, and provider-neutral disabled/fake media ports. |
| [ADR-0060](0060-social-catalog-bundles-feed-scripts-and-app-tier.md) | Proposed | Additive over ADR-0058: named social-primitive catalog with gesture-only consequential actions, hash-pinned community bundles with member-granular killbit, identifier-only feed-skeleton scripts hydrated host-side, and a reserved sandboxed-iframe app tier. |
| [ADR-0061](0061-crt-tube-pass.md) | Accepted | Landing CRT becomes a full tube pass in `crt.js` (aperture grille, halation, screen-derived raster, lit room, `uMotion`-gated animation) with a static face layer so DOM copy wears the raster; the monitor (curved glass, lip and moulding) drawn by the shader so the frame follows the tube; measured contrast cost 11.46:1 → 10.21:1 against a 7:1 floor, pinned by assertion. |
