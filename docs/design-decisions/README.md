...
| [ADR-0040](0040-verified-launch-and-platform-execution-contract.md) | Proposed | Execute the descriptor whose bytes were digested where the platform allows, and launch Windows `.cmd`/`.bat` extensions through an explicitly quoted interpreter rather than a shell flag. |
| [ADR-0041](0041-sandboxed-capability-providers.md) | Proposed | Ship Tier 2 capability providers as import-free WebAssembly modules, so a provider that shapes signed evidence is deterministic and holds no ambient authority. |
