# ADR-0028: Recoverable Nightboard Startup, Sticky Routing, And Deterministic HoBo Authoring

Status: Accepted

## Context

Nightboard power users need to resume work from other coding-agent sessions,
...
The current Nightboard is a static local browser app with one on-device model
...
| Alternative | Useful policy/config surface | Decision for Nightboard |
|---|---|---|
...
| [Portkey Gateway](https://portkey-docs.mintlify.dev/docs/product/ai-gateway) | Self-hosted gateway, simple/semantic cache, conditional routes, circuit breakers, budgets, and guardrails. | Appropriate when Nightboard needs shared gateway operations; unnecessary for one local browser session. |
...
- The same policy can later point at Switchyard, LiteLLM, Portkey, or a managed
  gateway without changing Nightboard's workspace/session identity.
...
Revisit when Nightboard gains two live provider transports, measured routing
...
- [Nightboard](../design-explorations/nightboard/README.md)
