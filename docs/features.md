...
| [`features/live_spaces.feature`](../features/live_spaces.feature) | Live Spaces: signed live-session lifecycle over an existing Space, fail-closed allow-list publication with immutable secret denials, observer joins and grant requests that never auto-grant, checkpoint annotation and exact-point forking, spectator gap recovery, and honest media capability labels. |
