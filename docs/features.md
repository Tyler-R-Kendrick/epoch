...
| [`features/community_web_experience.feature`](../features/community_web_experience.feature) | Canonical Nightboard landing-to-board entry, stable contextual links, mounted/saved projections, explicit thread versus namespace navigation, APG feed/tree/combobox keyboard behavior, deterministic `cd` versus global jump, safe shared actions, startup/routing/HoBo power controls, responsive navigation, signed actions, and moderation/legal-hold evidence. |
