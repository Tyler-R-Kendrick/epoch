...
| [`features/community_web_experience.feature`](../features/community_web_experience.feature) | Canonical Community Web plus Change Graph and deterministic discovery journeys: typed cross-source search with completeness/explain, Projection Definitions, repeated occurrence identity, scoped Namespace Mounts and recovery, keyset snapshots, browser fallback, GraphQL/text parity, atomic split, stable Revisions, Review Bundles, dependency-closed merge, durable conflicts, interoperability, authority, archival/privacy, accessible keyboard navigation, and self-evolving interface proposals with semantic diff, confirmed merge, harness-enforced recovery, and one command receipt across terminal and agent. |
