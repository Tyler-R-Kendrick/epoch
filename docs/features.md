...
| [`features/community_web_experience.feature`](../features/community_web_experience.feature) | Canonical Nightboard landing-to-board entry, CanvasUI motion, tmux-style keyboard collaboration, roving message and context-menu focus, keyboard-parity post actions including repost and share, addressable message directories with synchronized/cancellable `cd` preview, safe prompt/agent/voice actions, one Ctrl+U startup restart, cache-sticky workspace routing, deterministic Bo/HoBo authoring with trainable fallback, focused-panel expansion, responsive and 200%-zoom navigation, signed actions, agent requests, and moderation/legal-hold evidence. |
