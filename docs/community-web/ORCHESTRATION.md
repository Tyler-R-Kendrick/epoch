# Community Web Persuade landing — orchestration runbook

Live scoreboard: [`progress.html`](../../packages/Epoch.Community.Web/app/progress.html) (serve with the exploration).
Surface brief: [`.impeccable-surface-landing.md`](../../packages/Epoch.Community.Web/.impeccable-surface-landing.md).

This runbook implements the improved master prompt: iterate the **existing**
static Persuade landing under Community Web Grid. Do **not** recreate it as a
Higgsfield Worker / A4 film site.

## Authority (in order)

1. Root [`DESIGN.md`](../../DESIGN.md) + [ADR-0027](../design-decisions/0027-community-visual-world.md)
2. [`.impeccable-surface-landing.md`](../../packages/Epoch.Community.Web/.impeccable-surface-landing.md)
...
```bash
python3 -m http.server 8902 --directory packages/Epoch.Community.Web/app
# Landing:  http://127.0.0.1:8902/
# Progress: http://127.0.0.1:8902/progress.html
# Board:    http://127.0.0.1:8902/board.html
```

Or: `node packages/Epoch.Community.Web/scripts/serve.mjs`

## Canvas UI MCP (component registry)

Community Web uses Canvas UI vanilla effects ([installation](https://canvasui.dev/docs/installation),
...
```text
ROLE: Orchestrator for Epoch Community Web Persuade landing (NOT a greenfield Higgsfield site).

AUTHORITY (in order):
1. DESIGN.md + ADR-0027 Community Web Grid
2. packages/Epoch.Community.Web/app/.impeccable-surface-landing.md
...
SURFACE:
- Edit: packages/Epoch.Community.Web/app/{index.html,landing.css,landing.js} (+ ascii.js if brand)
- Serve static: python3 -m http.server …/packages/Epoch.Community.Web/app  (or existing serve.mjs)
...
ORCHESTRATION:
1. Write/update packages/Epoch.Community.Web/app/progress.html (simple live scoreboard:
...
7. /loop dynamic heartbeat: wake when progress.html changes OR every 15–20m fallback.
   ...
   n/a heuristics) AND gate:push + CW_E2E=landing: e2e green AND HCD automatic fails clear.
   Do NOT loop on “wow forever.”

GATES BEFORE “DONE”:
- npm run gate:push
- CW_E2E=landing: node packages/Epoch.Community.Web/test/e2e.mjs
...
If OG/cover assets needed: higgsfield generate … only; honor Community Web tokens;
...
Surface: Community Web Persuade landing (/)
...
- `CW_E2E=landing: node packages/Epoch.Community.Web/test/e2e.mjs` green
...
- Replacing Community Web with a Higgsfield A4 film marketing site
