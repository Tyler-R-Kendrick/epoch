...
| Mutation | Focused, not whole-program | PR-time oracles plus **source mutant kill** for NATS (`npm run mutation:nats`), XMPP (`npm run mutation:xmpp`), Protocol capability flags (`npm run mutation:protocol`), and Community Web Activity terminal nav (`npm run mutation:community-web`). Full-project Stryker scoring stays out of band | Quality Gates mutant-kill jobs |
