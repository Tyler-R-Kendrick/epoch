...
| Mutation | Focused, not whole-program | PR-time oracles plus **source mutant kill** for NATS (`npm run mutation:nats`), XMPP (`npm run mutation:xmpp`), and Protocol capability flags (`npm run mutation:protocol`). Full-project Stryker scoring stays out of band | Quality Gates `NATS mutant kill`, `XMPP mutant kill`, `Protocol mutant kill` |
...
- Full-project Stryker mutation scoring (NATS, XMPP, and Protocol use owned mutant-kill scripts instead)
...
- Chaos beyond NATS auth/svc, XMPP fanout envelopes, and Community Web AI-draft faults (no cluster-wide fault injection)
