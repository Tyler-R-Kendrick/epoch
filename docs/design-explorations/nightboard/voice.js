/**
 * Low-latency channel voice — Discord-parity WebRTC mesh for Nightboard.
 *
...
(function () {
  ...
  var CHANNEL = "epoch-nightboard-voice-v1";
  ...
  function create(opts) {
    ...
    async function join(channelId, channelPath) {
      ...
      state.error = null;
      handle = (opts.handle) ||
        (window.NB_APP && typeof window.NB_APP.getIdentity === "function" &&
          (function () {
            var id = window.NB_APP.getIdentity();
            return id && (id.handle || id.displayName);
          })()) ||
        (window.NB_APP && window.NB_APP.state && window.NB_APP.state.identity &&
          (window.NB_APP.state.identity.handle || window.NB_APP.state.identity.displayName)) ||
        "you";
