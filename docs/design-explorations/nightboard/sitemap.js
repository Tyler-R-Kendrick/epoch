...
(function () {
  ...
  function registeredProjectionPaths() {
    ...
    if (window.NB_WORKBENCH) window.NB_WORKBENCH.definitions()
  ...
  function projectionForPath(path) {
    ...
    var definition = parts[0] === "views" && parts[1] && window.NB_WORKBENCH
      ? window.NB_WORKBENCH.definitions().find(function (item) { return item.projectionId === parts[1]; }) : null;
    if (definition) {
  ...
  function projectionLocations(objectId, extra) {
    ...
    if (window.NB_WORKBENCH) {
      window.NB_WORKBENCH.definitions().forEach(function (definition) {
  ...
  function list(path, extra) {
    ...
    if (parts.length === 0) {
      ...
      var builtins = [
        ...
        { name: "views", kind: "dir", meta: "projections",
          hint: (window.NB_WORKBENCH ? window.NB_WORKBENCH.definitions().length : 0) + " Projection Definitions" },
    ...
    if (parts[0] === ".epoch") {
      ...
      if (parts.length === 2 && parts[1] === "projections") return (window.NB_WORKBENCH ? window.NB_WORKBENCH.definitions() : []).map(function (definition) {
        return { name: definition.projectionId, kind: "file", meta: "projection definition", hint: definition.label };
      });
      if (parts.length === 2 && parts[1] === "sources") return [{ name: "nightboard-host", kind: "file", meta: "current", hint: "resident fixture/live source" }];
    ...
    if (parts[0] === "views") {
      if (!window.NB_WORKBENCH) return parts.length === 1 ? [] : null;
      if (parts.length === 1) {
        return window.NB_WORKBENCH.definitions().map(function (definition) {
          return {
      ...
      if (parts.length === 2) {
        var selected = window.NB_WORKBENCH.definitions().find(function (definition) { return definition.projectionId === parts[1]; });
        if (!selected || !window.NB_QUERY) return null;
        var expression = projectionSelection(selected.root);
        return postEntries(allMessages(extra).filter(function (post) {
          return !expression || core().matchesNormalizedQuery(window.NB_MAP.toCommunityMessage(post), { ast: expression });
        }));
      }
      return null;
    }
