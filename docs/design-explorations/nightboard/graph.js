/** Nightboard host adapter for the portable @epoch/community-graphql runtime. */
(function () {
  "use strict";

  var D = window.NB_DATA;
  var MAP = window.NB_MAP;
  var CORE = window.NB_CORE;
  ...
  function visibleDefinitions(authorization) {
    var definitions = [CORE.builtinDefaultProjection].concat(window.NB_WORKBENCH ? window.NB_WORKBENCH.definitions() : []);
  ...
  function visibleMounts(authorization, definitions) {
    ...
    return (window.NB_WORKBENCH ? window.NB_WORKBENCH.mounts() : []).filter(function (mount) {
  ...
  function corpusKey(authorization) {
    return JSON.stringify({
      ...
      mounts: window.NB_WORKBENCH ? window.NB_WORKBENCH.mounts() : [],
    });
  }
  ...
  async function createState(authorization) {
    ...
    var search = new CORE.SearchService({
      ...
      sources: [{ sourceId: "nightboard-host", fields: fields, fullText: "lexical",
  ...
  function messageEntity(post, isDm, authorization) {
    ...
    return CORE.communityMessageToEntity(message, {
      provenance: { sourceId: "nightboard-host", nativeId: message.ref.objectId, observedAt: message.updatedAt || message.publishedAt },
  ...
  function genericEntity(ref, title, extraFields, visibility, ownerId) {
    ...
    return CORE.validateCommunityEntity({
      ...
      createdAt: D.board.observedAt,
      updatedAt: D.board.observedAt,
      provenance: { sourceId: "nightboard-host", nativeId: ref.objectId, observedAt: D.board.observedAt },
    });
  }
  ...
  var services = {
    ...
    saveProjection: async function (definition, authorization) {
      ...
      var state = {};
      window.NB_WORKBENCH.openProjection(state, Object.assign({}, definition, { ownerId: actor }));
      var saved = window.NB_WORKBENCH.saveProjection(state);
      clearStateCaches();
      return saved;
    },
    deleteProjection: async function (id, authorization) {
      ...
      var deleted = window.NB_WORKBENCH.deleteProjection(id);
      clearStateCaches();
      return deleted;
    },
    projectionContext: async function (authorization, snapshot) {
      ...
      return { authorizationFingerprint: state.authorizationFingerprint,
        snapshotId: snapshot || "nightboard-" + D.board.observedAt.replace(/[^0-9]/g, "") };
    },
    ...
    sourceCapabilities: async function (authorization) {
      ...
      return [{ sourceId: "nightboard-host", fields: fields, fullText: "lexical", pagination: "keyset",
    ...
    mountProjection: async function (input, authorization) {
      ...
      window.NB_WORKBENCH.mount(mount);
      clearStateCaches();
      return mount;
    },
    unmountProjection: async function (id, authorization) {
      var actor = requireActor(authorization);
      var mount = window.NB_WORKBENCH.mounts().find(function (item) { return item.mountId === id; });
      ...
      var removed = window.NB_WORKBENCH.unmount(id);
      clearStateCaches();
      return removed;
    },
    resetNamespace: async function (scope, authorization) {
      requireActor(authorization);
      var before = window.NB_WORKBENCH.mounts().filter(function (mount) { return mount.scope === scope; });
      window.NB_WORKBENCH.resetNamespace(scope);
      clearStateCaches();
  ...
  function scopedExpression(expression, scope) {
    ...
    if (scope.sourceIds && scope.sourceIds.some(function (sourceId) { return sourceId !== "nightboard-host"; })) {
