...
const outputDirectory = resolve(repository, "docs/design-explorations/nightboard");
const targets = [
...
for (const [source, filename] of targets) {
  const result = await build({
    ...
    platform: "browser",
    target: ["es2022"],
    banner: { js: `/* Generated from ${source}. Run npm run nightboard:build. */` },
    write: false,
  });
...
function publish(path, bytes) {
  if (check) {
    if (!readFileSync(path).equals(bytes)) throw new Error(`${path} is stale; run npm run nightboard:build`);
