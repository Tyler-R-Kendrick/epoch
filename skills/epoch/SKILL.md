...
| [Change Graph And Operation History](../../docs/change-graph.md) | Stable Changes/Revisions/graphs, conflicts, objects/sync/workspaces, Git/forge interop, agent authority, archival, and honest limits. |
...
- `Epoch.Platform.Web` is only the Epoch hosting control plane; `Epoch.Community.Web` is registered with Web through a deployable app descriptor, while Community API behavior lives in `Epoch.Community.API` and Web/CLI consume `Epoch.Community.Core`. The canonical Community browser runtime is Community Web (`/` landing, `/board.html` tmux-style board); use `npm run dev:community-web` locally and do not revive the former renderer as an app entrypoint.
...
- For Change Graph work, preserve stable opaque IDs and signed events; verify
