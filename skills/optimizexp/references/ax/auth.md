...
tags: [epoch, optimizexp, ax, auth, secrets, workos, auth-md]
...
for **agent registration** with applications — not an Epoch house style for “list our API keys.”
...
- Product identity/auth: Epoch identity federation port, agent credentials design, XAA / ID-JAG where
...
- When Epoch ships a public app/API agents call, the **auth.md / AUTH.md** for that origin should
...
4. Never imply WorkOS endorsement of Epoch; naming the open protocol is nominative fair use.
...
| Local MCP/tool keys for humans hacking on Epoch | Root `AUTH.md` residual tables + `.env.example` |
...
| AUTH.md mislabeled as an Epoch-only invention in docs | uncertainty ≥ 2 (wrong mental model) |
