...
- **Receipt locators are sacred chrome.** `button.cn-sig-text[data-receipt-locator]` (`sig:…` / `intent://…`) must keep Bracket Rule styles + `--cw-signed` (never native button chrome). `npm run community-web:app:design-lint` gates this; see `.cursor/rules/community-web-receipt-chips.mdc`.
